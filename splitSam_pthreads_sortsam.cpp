...
void *Process_read(void *arg)
{
	...
	while( (!bamformat && (samaddress = fgets(s2t,BATBUF,((ARGS *)arg)->samINFILE))!=NULL) || (bamformat && r>0 ))
	{
		...
		*/
		if ( fileprocess>=1000000/NTHREAD && ( ((ARGS *)arg)->ThreadID==1 || !NTHREAD ) ) {
		...
		if(strcmp(processingchr.c_str(), Chrom) != 0) {
	        ...
	        if(bamformat)
	        	if(r <= 0) fileEnd=1;
	        else if(samaddress==NULL) fileEnd=1;
	        while(NthreadFlag != 0 && NthreadFlag != NTHREAD && !fileEnd)  {
	            //fprintf(stderr, "thread wait %d %d\n", ((Param *)arg)->ThreadID, NthreadFlag);
	            gettimeofday(&now, NULL);
	            outtime.tv_sec = now.tv_sec + 10;
	            pthread_cond_timedwait(&cond, &mark_mutex, &outtime);
	            //fprintf(stderr, "thread reruning %d %d\n", ((Param *)arg)->ThreadID, NthreadFlag);
	        }
	        if(NthreadFlag == NTHREAD || NTHREAD==0){
